...
setMethod(
    ...
    function(x, usePlotly = FALSE, labels, ...){
        ...
        df <- tidyr::gather(
            df, key = "Base", value = "Percent", tidyselect::one_of(acgt)
        )
        ...
        scPlot <- ggplot(
            df,
            aes_string("x", "Percent", label = "Position", colour = "Base")
        ) +
            geom_line() +
            facet_wrap(~Filename) +
            scale_y_continuous(
...
setMethod(
    ...
    function(
        ...
        if (plotType == "heatmap") {
            ...
            maxBase <-
                max(vapply(acgt, function(x){max(df[[x]])}, numeric(1)))
            ...
            if (usePlotly) {
                ...
                status <- getSummary(x)
                status <-
                    subset(status, Category == "Per base sequence content")
