...
setMethod(
    ...
    function(
        ...
        if (plotType == "histogram") {
            ...
            p <- ggplot(
                ...
                aes(
                    ...
                    cumperc = !!sym("Cumulative %"),
                    cumtot = !!sym("Cumulative Total")
                )
            ) +
                geom_col(...) +
        ...
        if (plotType == "cumulative") {
            ...
            main <- paste0(
                unique(df$Filename), ": Cumulative Insert Size Distribution"
            )
            p <- ggplot(
                df,
                aes(
                    !!sym("Insert Size"), !!sym(yval),
                    percent = !!sym("%"), total = Total,
                    cumperc = !!sym("Cumulative %"),
                    cumtot = !!sym("Cumulative Total")
                )
        ...
        if (usePlotly) {
            tt <- c(
                "Filename", "x", "Total", "Cumulative Total", "%",
                "Cumulative %"
            )
...
setMethod(
    ...
    function(
        ...
        scaleFill = NULL, scaleColour = NULL, cluster = FALSE,
        dendrogram = FALSE, heat_w = 8, ...
    ){
        ...
        if (plotType %in% c("line", "cumulative")) {
            ...
            df <- mutate(
                df, across(all_of(unname(vals)), \(x) round(100 * x, 3))
            )
        ...
        if (plotType == "heatmap") {
            ...
            if (n > 1) {
                clusterDend <- .makeDendro(
                    df, "Filename", "Insert Size", "Frequency"
                )
