...
setMethod(
    ...
    function(x, usePlotly = FALSE, labels, pwfCols, warn = 25, fail = 20,
        ...
        x <- getFastqcData(x)
        plotBaseQualities(
            x, usePlotly, labels, pwfCols, warn, fail, boxWidth, ...
        )
    }
)
...
setMethod(
    ...
    function(
        ...
        if (plotType == "heatmap") {
            ...
            plotValue <-
                ifelse(missing(plotValue), "Mean", match.arg(plotValue))
            ...
            if (cluster) {
                clusterDend <-
                    .makeDendrogram(df, "Filename", "Start", plotValue)
            ...
            phredMax <-
                ifelse(maxVal <= warn, max(maxQ, 41), ceiling(maxVal + 1))
            ...
            if (usePlotly) {
                ...
                status <- getSummary(x)
                status <-
                    subset(status, Category == "Per base sequence quality")
                ...
                qualPlot <-
                    plotly::layout(qualPlot, xaxis3 = list(title = xlab))
            }
            else{
