...
setMethod(
    ...
    function(x, usePlotly = FALSE, labels, pwfCols, warn = 5, fail = 10,  ...){
        ...
        acPlot <- ggplot(df) +
            ...
            geom_line(
                aes_string(x = "Position", y = "Percent", colour = "Type")
            ) +
...
setMethod(
    ...
    function(
        ...
        adapterType <- grep(adapterType[1],
                            c("Total_Adapter_Content", valueCols),
                            value = TRUE)
        if (length(adapterType) != 1) stop(
            "Could not determine adapter type"
        )
        ...
        if (plotType == "line") {
            ...
            if (cluster){
                clusterDend <-
                    .makeDendrogram(df, "Filename", "Position", "Percent")
                key <- labels(clusterDend)
            }
            df$Filename <- factor(labels[df$Filename], levels = labels[key])
