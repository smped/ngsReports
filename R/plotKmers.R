...
#' @param fillScale,colourScale ggplot2 scales to be used for colour palettes
...
#'   fillScale = scale_fill_gradient(low = "white", high = "black")
#' )
#'
#' @docType methods
#'
...
setMethod(
  ...
  function(
    x, usePlotly = FALSE, labels, pattern = ".(fast|fq|bam).*",
    n = 6, linewidth = 0.5, plotlyLegend = FALSE, colourScale = NULL,
  ...
  if (is.null(colourScale)) {
    if (!is.null(pal)) {
      pal <- rep_len(pal, n)
      colourScale <- scale_colour_manual(values = pal)
    }
...
setMethod(
  ...
  function(
    ...
    fillScale = NULL, heatCol = hcl.colors(50, "inferno"), heat_w = 8, ...
  ){
    ...
    if (is.null(fillScale)) {
      if (!is.null(heatCol)) {
        fillScale <- scale_fill_gradientn(colors = heatCol)
      } else {
        fillScale <- scale_fill_viridis_c(option = "inferno")
      }
    }
...
setMethod(
  ...
  function(
    ...
    trans = "log2", fillScale = NULL, plotTheme = theme(),
    plotlyLegend = FALSE, ...
  ){
    ...
    if (is.null(fillScale)) fillScale <- scale_fill_gradient2()
    stopifnot(is(fillScale, "ScaleContinuous"))
    stopifnot(fillScale$aesthetics == "fill")
