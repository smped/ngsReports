...
setMethod(
    ...
    function(
        ...
        df <- dplyr::distinct(
            df, Filename, Sequence, Position, .keep_all = TRUE
            )
        ...
        p <- ggplot(df, aes(Position, Value, colour = Sequence)) +
            ...
            facet_wrap(~Filename) +
            scale_x_continuous(
                breaks = refForX$Position, labels = refForX$Base,
                expand = c(0.02, 0)
            ) +
...
setMethod(
    ...
    function(
        ...
        if (!is.null(trans)) {
            f <- match.fun(trans)
            stopifnot(length(f(seq_len(2))) == 2)
        ...
        p <- ggplot(
            df,
            aes(
                x = !!sym("suffix"), y = !!sym("prefix"),
                fill = !!sym("times_mean"), count = !!sym("count")
            )
        ) +
            geom_raster() +
