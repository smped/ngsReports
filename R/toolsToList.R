...
#'
#' @param Dir A directory containing log files
...
writeNgsReport <- function(
    ...
    if (!dir.exists(Dir)) stop("Could not find directory", Dir)
    cp <- file.copy(template, Dir, overwrite = overwrite)
    if (!cp) {
        msg <- paste(
    ...
    file2Knit <- file.path(Dir, basename(template))
    ...
    ## get all tool types in directory
    toolList <- ngsReports:::.toolsToList(x)
    
    ## Compile the document in the directory
    htmlOut <- file.path(Dir, gsub(".Rmd$", ".html", basename(template)))
...
.toolsToList <- function(x){
    ...
    type <- unlist(lapply(data, function(y){ngsReports:::.getToolName(y, possTypes = possTypes)}))
    
    typeList <- split(names(type), type)
