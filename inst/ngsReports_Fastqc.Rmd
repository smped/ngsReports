...
plotBaseQualities(rawFastqcData, labels = plotLabels, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram, usePlotly = globals$usePlotly)
```

## Per Sequence Quality Scores

```{r plotSequenceQualitiesHeatmap, fig.cap = "Heatmap showing mean sequence qualities for each library"}
plotSequenceQualitiesHeatmap(rawFastqcData, labels = plotLabels, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram, usePlotly = globals$usePlotly)
```

## Per Base Sequence Content

```{r plotSequenceContent, fig.cap = "Heatmap of summed base distributions along each read"}
plotSequenceContent(rawFastqcData, labels = plotLabels, usePlotly = globals$usePlotly)
```

## Per Sequence GC Content

```{r plotGCHeat, fig.cap = "GC Content Heatmap"}
plotGcContent(rawFastqcData, labels = plotLabels, theoreticalGC = globals$theoreticalGC, theoreticalType = globals$theoreticalType, species = globals$species, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram, usePlotly = globals$usePlotly)
```


```{r plotGCLine, fig.cap = "GC Content Distributions for all reads"}
...
plotSequenceLengthDistribution(rawFastqcData, labels = plotLabels, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram, usePlotly = globals$usePlotly)
```

## Sequence Duplication Levels

```{r plotDuplicationLevels}
plotDuplicationLevels(rawFastqcData, labels = plotLabels, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram, usePlotly = globals$usePlotly)
```

## Overrepresented Sequences

```{r Overrepresented_sequences, echo=FALSE, results='asis'}
nOver <- 30
Overrepresented_sequences(rawFastqcData) %>% 
...
plotAdapterContent(rawFastqcData, labels = plotLabels, adapterType = "Universal", plotType = "line", usePlotly = globals$usePlotly) 
```

## Kmer Content

```{r plotKmerHeatmap}
plotKmerHeatmap(rawFastqcData, labels = plotLabels, usePlotly = globals$usePlotly, clusterNames = globals$clusterNames, dendrogram = globals$dendrogram)
```

```{r Kmer_Content}
nK <- 30
Kmer_Content(rawFastqcData) %>%
	arrange(desc(Count)) %>%
	dplyr::slice(1:nK) %>%
	arrange(Sequence) %>%
	mutate(Filename = plotLabels[Filename]) %>%
